...
for site in sites:
    ...
    try:
        ...
        articles_to_process = paper.articles[:8]
        ...
        for article_obj in articles_to_process:
            try:
                ...
                all_news.append({
                    ...
                    "published": pub_str,
                    "summary": (
                        article_obj.summary[:300] + "..."
                        if article_obj.summary
                        else article_obj.text[:300] + "..."
                    ),
                    "text_preview": (
                        article_obj.text[:400] + "..."
                        if article_obj.text else ""
                    ),
                    "source": site["name"],
...
print(f"Collected {len(all_news)} articles! Saved to {output_file}")
